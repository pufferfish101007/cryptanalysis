...
decipher(); // when things first load, if 'reverse?' is true then we need to update the plaintext before it tries to copy the plaintext into the ciphertext, otherwise we'll end up with a blank ciphertext.
watchEffect(() => {
...
const quickKeywordSub = (e) => {
  const key = Array.from(new Set(e.target.key.value.toLowerCase().split(''))).join('');
  e.target.key.value = key;
  function* letterGen() {
    const lastLetterIndex = key.at(-1).charCodeAt(0) - 97
    let i = (lastLetterIndex + 1) % 26;
  ...
  const letters = letterGen();
  for (const i in info.value.subletters) {
    info.value.subletters[i] = i < key.length ? key[i] : letters.next().value;
  }
}
...
<template>
  ...
  <div class="container">
    <div id="ciphermodeselect">
      ...
      <template v-for="[id, display] in modesEnum" :key="id">
        <input type="radio" v-model="info.ciphermode" :id="`mode-${id}`" :value="id" name="ciphermode" />
        <label :for="`mode-${id}`">{{ display }}</label>
      </template>
    </div>
  </div>

  <!-- Basic stats + 'quick access' actions -->
  <div class="container">
    <div>
      <label>
    ...
    <div>
      <div>
        ...
        <Info>A monogram fitness close to 1 is similar to standard English; close
          to 0 is not similar.</Info>
      </div>
      <div>
        Tetragram fitness: {{ tetragramFitness(info.ciphertext).toFixed(2) }}
        <Info>Less negative is a better fit to English; more negative is a worse
          fit.</Info>
      </div>
      <div>
      ...
      <div>
        ...
        <Info>A higher entropy value means it is more random and less
          predictable.</Info>
      </div>
    </div>
    <div v-if="info.ciphermode === 'polyalphabetic'">
      ...
      <form @submit.prevent="quickVigenere">
        <label>vigenère: <input type="text" name="key" style="max-width: 4em" /></label>
    ...
    <div v-if="info.ciphermode === 'monoalphabetic'">
      ...
      <form @submit.prevent="quickCaesar">
        <label>caesar:
          <input type="number" name="key" value="3" style="max-width: 4em" /></label>
        <button type="submit">go</button>
      </form>
      <form @submit.prevent="quickKeywordSub">
        <label>keyword subsitution:
          <input type="text" name="key" value="" style="max-width: 4em" /></label>
        <button type="submit">go</button>
      </form>
      <button @click="
    ...
    <div v-if="info.ciphermode === 'polyalphabetic'">
      <div>
        <label>period: <input type="number" v-model="info.polyalphabeticPeriod" /></label>
      </div>
      <div>
        probable period: {{ info.probablePeriod || 'unknown' }}
        <button @click="calcProbablePeriod">recalculate</button>
        <button v-if="info.probablePeriod > 0" @click="info.polyalphabeticPeriod = info.probablePeriod">
          use this period
        </button>
      </div>
    </div>
  ...
  <div class="container">
    <div>
      <button v-if="info.ciphermode === 'monoalphabetic' && !info.encoding" @click="hillClimb('monoalphabetic')">
        <!-- prettier-ignore -->
        carry out stochastic hill climbing attack for monoalphabetic substitution cipher
      </button>
      <template v-else-if="info.ciphermode === 'polyalphabetic' && !info.encoding">
        <button @click="hillClimb('polyalphabetic')">
          <!-- prettier-ignore -->
  ...
  <div class="container">
    <div class="container vertical">
      ciphertext:<textarea v-model="info.ciphertext" :disabled="info.encoding"></textarea>
    </div>
    <div class="container vertical">
      plaintext:<textarea v-model="plaintext" :disabled="!info.encoding"></textarea>
    </div>
  </div>
  ...
  <!-- more in-depth stats (e.g. letter frequencies), + key for substitution ciphers -->
  <div class="container">
    <details open>
      <summary>letter frequencies</summary>
      <Table :columns="letterFreqsColumns" :data="letterFreqsData">
        <!-- prettier-ignore -->
        <template v-for="i in info.polyalphabeticPeriod" #[`subletter${i-1}`]="{ data, primaryKey }">
          <div contenteditable @keydown.enter.prevent="$event.target.blur()" @input="subLetterInput($event, primaryKey)"
            @blur="subLetterBlur($event, primaryKey)" class="subletter-input">
            {{ data }}
          </div>
        </template>
      </Table>
  ...
  <Modal ref="thresholdModal" :closeonblur="false" :close-buttons="['cancel', 'ok']"
    ...
    <template v-if="info.ciphermode === 'polyalphabetic'">
      ...
      <label v-if="info.assumeVigenere">
        initial guess <input type="text" v-model="info.vigenereInitialGuess" :minlength="info.polyalphabeticPeriod"
          :maxlength="info.polyalphabeticPeriod" />
      </label>
    </template>
  </Modal>
  <Modal ref="processingModal" :closeonblur="true" :close-buttons="[]"><span id="processing-span">processing</span>
  </Modal>
  <Modal ref="noticeModal" closeonblur>
    {{ noticeModalMsg }}
  </Modal>
  <Modal ref="probablePeriodModal" :close-buttons="['cancel', 'ok']" @close="(v) => {
    if (v === 'ok') {
