<script setup>
      ...
      [
        ...
        info.value.ciphermode === 'monoalphabetic'
          ...
          : info.value.ciphermode === 'polyalphabetic'
            ? Array.from(
                { length: info.value.polyalphabeticPeriod },
                (_, i) => ({
                  key: `subletter${i}`,
  ...
  function decipher() {
    ...
    switch (info.value.ciphermode) {
      ...
      case 'column':
        ...
        plaintext.value = decipherColumnarTransposition(
          c,
  ...
  watchEffect(() => {
    ...
    switch (info.value.ciphermode) {
      ...
      case 'morse':
        ...
        break;
      case 'column':
        info.value.ciphertext = encipherColumnarTransposition(
          p,
          info.value.permutation,
        );
        break;
  ...
  const sendHillClimb = (type) => {
    ...
    for (let i = 0; i < info.value.hillClimbThreads; i++) {
      ...
      hillClimbWorker.postMessage({
        ...
        initialGuess: info.value.assumeVigenere
          ? info.value.vigenereInitialGuess
              .toLowerCase()
              .split('')
  ...
  const quickKeywordSub = (e) => {
    const key = Array.from(
      new Set(e.target.key.value.toLowerCase().split('')),
    ).join('');
...
<template>
  ...
  <div class="container">
    ...
    <div v-if="info.ciphermode === 'monoalphabetic'">
      ...
      <form @submit.prevent="quickKeywordSub">
        <label
          >keyword subsitution:
          <input type="text" name="key" value="" style="max-width: 4em"
        /></label>
        <button type="submit">go</button>
      </form>
      <button
  ...
  <div class="container">
    <div>
      ...
      <button
        v-else-if="info.ciphermode === 'playfair' && !info.encoding"
        @click="hillClimb('playfair')"
      >
  ...
  <div class="container">
    <div v-if="['permutation', 'column'].includes(info.ciphermode)">
      <label for="permutationText">Key:</label>
      <input type="text" v-model="permutationText" id="permutationText" />
    </div>
  ...
  >
    <template
      v-if="
        ['monoalphabetic', 'polyalphabetic', 'playfair'].includes(
          info.ciphermode,
        )
      "
    >
    ...
    <template v-if="info.ciphermode === 'polyalphabetic'">
      ...
      <label v-if="info.assumeVigenere">
        ...
        <input
          type="text"
          v-model="info.vigenereInitialGuess"
          :minlength="info.polyalphabeticPeriod"
  ...
  <Modal ref="processingModal" :closeonblur="true" :close-buttons="[]"
    ><span id="processing-span">processing</span>
  </Modal>
